...
## Load COVID trajectory groups  

traj_path="//ad.bu.edu/bumcfiles/SPH/Projects/SC/06Methods_and_Specifications/05CWBI2020/Internal/03Analytic/ModeAdj_Scoring_Ranks/covid19/07122021/PublishCodes/"

traj1 <- read.csv(paste0(traj_path,"/t1/class_period1.csv"), stringsAsFactors = FALSE)
traj2 <- read.csv(paste0(traj_path,"/t2/class_period2.csv"), stringsAsFactors = FALSE)
traj3 <- read.csv(paste0(traj_path,"/t3/class_period3.csv"), stringsAsFactors = FALSE)

names(traj1)[2] <- "p1_class5"  # rename for 5 classes, 3/12/25
names(traj2)[2] <- "p2_class4"
names(traj3)[2] <- "p3_class4"

traj <- merge(traj1, traj2, by = "FIPS_CODE")
traj <- merge(traj, traj3, by = "FIPS_CODE") # 3142*4  3/12/25

traj$FIPS_CODE <- unlist(lapply(traj$FIPS_CODE, function(x) county_fips_fix(x)))


## COVID cases - National, longitudinal (and updated values from Nov 2020)  

covid_nat_sub <- read.csv(paste0(traj_path,"/Covid19_County_Cases__Daily_Update_11022020.csv"), stringsAsFactors = FALSE)
# dim(covid_nat_sub) 3142*13
covid_nat_sub <- covid_nat_sub[,c("FIPS_CODE", "newcases_p1", "newcases_p2", "newcases_p3", "IncidenceRate")]
names(covid_nat_sub)[1] <- "FIPS_CODE"  # 3142*3


## Load SDOH scores  

scores <- read_excel("//ad.bu.edu/bumcfiles/bedacfiles/SCData/cWBIData/2020/sdoh_county_2020_updatedcovid.xlsx")
scores <- plyr::rename(scores, c("fips_code" = "FIPS_CODE"))  # 3142*2  3/12/25
scores <- plyr::rename(scores, c("sdoh" = "sdoh2"))

## Load ACS demographics 

dem <- read_excel("//ad.bu.edu/bumcfiles/SPH/Projects/SC/08Data/03Analytic_Data/01ReferenceData/01Demographic/county_demographic_2020_updatedcovid.xlsx")
# 3142*2 3/12/25  county:"48243" missing in median household income
...
urban$ur_group <- urban$RUclass
urban$ur_group[which(urban$RUclass == 1)] <- "urban"
urban$ur_group[which(urban$RUclass == 0)] <- "rural"

urban_part <- urban[,c("FIPS_CODE","ur_group")]  # 3142*2  3/12/25



## Census regions and divisions 

cw <- read_excel("//ad.bu.edu/bumcfiles/SPH/Projects/SC/08Data/01Raw_Data/01ReferenceSets/31Relational_Files/crosswalk_files/Census_Region_Division/census_state_region.xlsx")
# 51*4  3/12/25

## Make sure county FIPS codes align, and no leading zeros are dropped

covid_nat_sub$FIPS_CODE <- unlist(lapply(covid_nat_sub$FIPS_CODE, function(x) county_fips_fix(as.character(x))))
scores$FIPS_CODE <- unlist(lapply(scores$FIPS_CODE, function(x) county_fips_fix(as.character(x))))
dem$FIPS_CODE <- unlist(lapply(dem$FIPS_CODE, function(x) county_fips_fix(as.character(x))))
urban_part$FIPS_CODE <- unlist(lapply(urban_part$FIPS_CODE, function(x) county_fips_fix(as.character(x))))


## Merge county level dataset

dat <- merge(covid_nat_sub, scores, by = "FIPS_CODE", all = TRUE)
dat <- merge(dat, dem, by = "FIPS_CODE", all = TRUE)
dat <- merge(dat, urban_part, by = "FIPS_CODE", all = TRUE)
dat <- merge(dat, traj, by = "FIPS_CODE", all = TRUE)
# dim(dat) 3142*12  3/12/25

## Create state indicator   

state_names <- read_excel("//ad.bu.edu/bumcfiles/SPH/Projects/SC/08Data/01Raw_Data/01ReferenceSets/31Relational_Files/crosswalk_files/state_name_fips.xlsx")
state_names <- state_names[,c(2,3)]
names(state_names) <- c("STFIPS", "state")

dat$STFIPS <- substr(dat$FIPS_CODE, 1,2)
dat <- merge(dat, state_names, by = "STFIPS", all.x = TRUE) # only for 3142 3/12/25
# 3142*14  3/12/25

## Ensure state is standardized to add Census region/division

dat$state <- tolower(dat$state)
cw$state <- tolower(cw$State)

dat <- left_join(dat, cw[,c("state", "Division","Region")], by = "state")
dat <- dat[which(!is.na(dat$Region)),]  

## Re-categorize SDoH 2.0 variable(s) as quantiles 

quants <- as.numeric(quantile(dat$sdoh2, c(0, 0.10, 0.50, 0.90, 1)))
dat$sdoh2_quant <- dat$sdoh2
dat$sdoh2_quant[which(dat$sdoh2 < quants[2])] <- 1
dat$sdoh2_quant[which(dat$sdoh2 >= quants[2] & dat$sdoh2 < quants[3])] <- 2
dat$sdoh2_quant[which(dat$sdoh2 >= quants[3] & dat$sdoh2 < quants[4])] <- 3
dat$sdoh2_quant[which(dat$sdoh2 >= quants[4])] <- 4



## Re-categorize trajectory classes for easier interpretation   

# 1=>1 4=>2 2=>3 5=>4 3=>5   4/7/25 period - 1

dat$p1_class5_save <- dat$p1_class5
dat$p1_class5[which(dat$p1_class5_save == 1)] <- 1
dat$p1_class5[which(dat$p1_class5_save == 4)] <- 2
dat$p1_class5[which(dat$p1_class5_save == 2)] <- 3
dat$p1_class5[which(dat$p1_class5_save == 5)] <- 4
dat$p1_class5[which(dat$p1_class5_save == 3)] <- 5

# 2=>1 3=>2 1=>3  4=>4   4/7/25 period -2

dat$p2_class4_save <- dat$p2_class4
dat$p2_class4[which(dat$p2_class4_save == 2)] <- 1
dat$p2_class4[which(dat$p2_class4_save == 3)] <- 2
dat$p2_class4[which(dat$p2_class4_save == 1)] <- 3
dat$p2_class4[which(dat$p2_class4_save == 4)] <- 4

# 1=>1 3=>2 4=>3  2=>4   4/7/25 period-3

dat$p3_class4_save <- dat$p3_class4
dat$p3_class4[which(dat$p3_class4_save == 1)] <- 1
dat$p3_class4[which(dat$p3_class4_save == 3)] <- 2
dat$p3_class4[which(dat$p3_class4_save == 4)] <- 3
dat$p3_class4[which(dat$p3_class4_save == 2)] <- 4


# save data for further tables and figures

save(dat, file=paste0(traj_path,"dat.RData"))
...
out_path="//ad.bu.edu/bumcfiles/SPH/Projects/SC/06Methods_and_Specifications/05CWBI2020/Internal/03Analytic/ModeAdj_Scoring_Ranks/covid19/07122021/PublishCodes/output/"
